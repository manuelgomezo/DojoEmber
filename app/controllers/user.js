...
export default Ember.Controller.extend({
  ...
  actions: {
    refreshPost: function (id) {
      var thiz = this;
      this.get('ajax').request(mainUrl+'/user/' + id).then(function success(data) {
        thiz.set('model', data);
      }, function error(err) {
    ...
    savePost: function (currentPost) {
      var thiz = this;
      this.get('ajax').request(mainUrl+'/user/' + currentPost.id, {
        method: 'PUT',
        data: currentPost
    ...
    deletePost(id) {
      var thiz = this;
      this.get('ajax').request(mainUrl+'/user/' + id,{
        method: 'DELETE'
      }).then(function success(data) {
          alert('Action completed')
          thiz.transitionToRoute('list');
